...
# Define a function to predict dependency score with one gene's fingerprint removed at a time
...
if __name__ == '__main__':
    ...
    predictions = []
    mastl_index = gene_names_fprint.index("MASTL")

    # MASTL için dependency skorlarını kaydedeceğimiz liste
    mastl_dependency_scores = []

    # İlk olarak, verileri değiştirilemez hale getirmek için bir kopya alıyoruz
    original_data_mut = np.copy(data_mut_tcga)
    original_data_exp = np.copy(data_exp_tcga)
    original_data_cna = np.copy(data_cna_tcga)
    original_data_meth = np.copy(data_meth_tcga)
    original_data_fprint = np.copy(data_fprint_1298DepOIs)

    for z in np.arange(0, first_to_predict):
        # Her bir geni sıfırlayarak MASTL geninin dependency skorunu hesapla
        for gene_index in tqdm(range(1298)):
            # Tüm genlerin orijinal verilerini geri yükle
            data_mut_tcga = np.copy(original_data_mut)
            data_exp_tcga = np.copy(original_data_exp)
            data_cna_tcga = np.copy(original_data_cna)
            data_meth_tcga = np.copy(original_data_meth)
            data_fprint_tcga = np.copy(original_data_fprint)

            # İlgili geni sıfırla
            data_mut_tcga[:, gene_index] = 0
            data_exp_tcga[:, gene_index] = 0
            data_cna_tcga[:, gene_index] = 0
            data_meth_tcga[:, gene_index] = 0
            data_fprint_tcga[:, gene_index] = 0

            mut_tensor = torch.tensor(data_mut_tcga[np.repeat(z, data_fprint_1298DepOIs.shape[0])], dtype=torch.float32).to(device)
            exp_tensor = torch.tensor(data_exp_tcga[np.repeat(z, data_fprint_1298DepOIs.shape[0])], dtype=torch.float32).to(device)
            cna_tensor = torch.tensor(data_cna_tcga[np.repeat(z, data_fprint_1298DepOIs.shape[0])], dtype=torch.float32).to(device)
            meth_tensor = torch.tensor(data_meth_tcga[np.repeat(z, data_fprint_1298DepOIs.shape[0])], dtype=torch.float32).to(device)
            fprint_tensor = torch.tensor(data_fprint_1298DepOIs, dtype=torch.float32).to(device)
    ...
    mastl_dependency_scores_df.to_csv(f"PytorchStaticSplits/DeepDepVAE/Analysis/mastl_dependency_scores_zeroed_genes.txt", sep='\t', index_label='Gene', float_format='%.4f')
