...
def train_model(model, train_loader, test_loader, num_epoch, patience, learning_rate, split_num):
    ...
    for epoch in range(num_epoch):
        ...
        if test_loss < best_loss:
            ...
            torch.save(best_model_state_dict, f'PytorchStaticSplits/DeepDepVAE/Results/Split{split_num}/PredictionNetworkModels/best_model_vae_split_{split_num}_LAST_AfterDecision.pth')
            print("Model saved")
...
if __name__ == '__main__':
    ...
    for split_num in range(1, 6):
        ...
        run = wandb.init(project="Latest-Model-VAE-PredictionNetwork", entity="kemal-bayik", name=f"Prediction_Network_{current_time}_VAE_Split_{split_num}")
        ...
        np.savetxt(f'PytorchStaticSplits/DeepDepVAE/Results/Split{split_num}/predictions/y_true_train_CCL_VAE_Split_{split_num}_LAST_AfterDecision.txt', y_true_train, fmt='%.6f')
        np.savetxt(f'PytorchStaticSplits/DeepDepVAE/Results/Split{split_num}/predictions/y_pred_train_CCL_VAE_Split_{split_num}_LAST_AfterDecision.txt', y_pred_train, fmt='%.6f')
        np.savetxt(f'PytorchStaticSplits/DeepDepVAE/Results/Split{split_num}/predictions/y_true_test_CCL_VAE_Split_{split_num}_LAST_AfterDecision.txt', y_true_test, fmt='%.6f')
        np.savetxt(f'PytorchStaticSplits/DeepDepVAE/Results/Split{split_num}/predictions/y_pred_test_CCL_VAE_Split_{split_num}_LAST_AfterDecision.txt', y_pred_test, fmt='%.6f')
