...
def plot_density(y_true_train, y_pred_train, y_true_test, y_pred_test, batch_size, learning_rate, epochs):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'results/predictions/dependency_score_density_plot_{batch_size}_{learning_rate}_{epochs}_VAE.png')
    plt.show()
...
def plot_results(y_true_train, y_pred_train, y_true_test, y_pred_test, batch_size, learning_rate, epochs):
    ...
    plt.plot(np.unique(y_pred_train), np.poly1d(np.polyfit(y_pred_train, y_true_train, 1))(np.unique(y_pred_train)), color='red')
    ...
    plt.plot(np.unique(y_pred_test), np.poly1d(np.polyfit(y_pred_test, y_true_test, 1))(np.unique(y_pred_test)), color='red')
    plt.xlabel('DeepDEP-predicted score')
    plt.ylabel('Original dependency score')
    plt.title(f'Testing\nBatch Size: {batch_size}, Learning Rate: {learning_rate}, Epochs: {epochs} VAE')
...
if __name__ == '__main__':
    ...
    dims_fprint = (data_fprint.shape[1], 1000, 200, 50)

    # Load pre-trained VAE models    
    premodel_mut = load_pretrained_vae('results/variational_autoencoders/premodel_ccl_mut_vae_demo.pickle', *dims_mut)
    premodel_exp = load_pretrained_vae('results/variational_autoencoders/premodel_ccl_exp_vae_demo.pickle', *dims_exp)
    premodel_cna = load_pretrained_vae('results/variational_autoencoders/premodel_ccl_cna_vae_demo.pickle', *dims_cna)
    premodel_meth = load_pretrained_vae('results/variational_autoencoders/premodel_ccl_meth_vae_demo.pickle', *dims_meth)
    premodel_fprint = load_pretrained_vae('results/variational_autoencoders/premodel_ccl_fprint_vae_demo.pickle', *dims_fprint)
    ...
    # Save the best model
    torch.save(best_model_state_dict, 'vae_model_demo.pth')
