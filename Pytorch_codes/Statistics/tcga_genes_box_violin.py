...
# Tüm kanser türleri için ortalama skorları içerecek DataFrame oluştur
all_cancer_averages = pd.DataFrame()

for cancer_type in cancer_types:
    cancer_type_path = f'Data/CancerTCGAMappings/{cancer_type}.txt'
    with open(cancer_type_path, 'r') as file:
        cancer_tcga_ids = file.read().splitlines()

    # İlgili TCGA ID'leri ile filtrele
    filtered_data = data[['CRISPR_GENE'] + cancer_tcga_ids]

    # Genlerin ortalama dağılımını hesapla
    mean_distribution = filtered_data.mean(axis=1)
    mean_distribution.name = cancer_type  # Seri adını kanser türü yap

    # DataFrame'e ekle
    all_cancer_averages = pd.concat([all_cancer_averages, mean_distribution], axis=1)

# CRISPR_GENE sütununu tekrar DataFrame'e ekleyelim
all_cancer_averages['CRISPR_GENE'] = filtered_data['CRISPR_GENE']

# Veriyi uzun formata çevir
all_cancer_averages_long = all_cancer_averages.reset_index().melt(id_vars='CRISPR_GENE', var_name='Cancer_Type', value_name='Average_Dependency_Score')

# Box plot çiz
plt.figure(figsize=(18, 10))
sns.boxplot(x='Cancer_Type', y='Average_Dependency_Score', data=all_cancer_averages_long)
plt.title('Average Dependency Scores Across All Cancer Types')
plt.xticks(rotation=90)
plt.xlabel('Cancer Type')
plt.ylabel('Average Dependency Score')
