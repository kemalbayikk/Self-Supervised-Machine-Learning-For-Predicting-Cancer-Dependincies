...
class VariationalAutoencoder(nn.Module):
    def __init__(self, input_dim, first_layer_dim, second_layer_dim, latent_dim):
        ...
        # Attention mechanism
        self.attention = nn.MultiheadAttention(embed_dim=second_layer_dim, num_heads=1)
...
if __name__ == '__main__':
    ...
    wandb.init(project="Self-Supervised-Machine-Learning-For-Predicting-Cancer-Dependencies", entity="kemal-bayik", name=f"TCGA_{omic}_{current_time}_attention")
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss < best_loss:
            ...
            save_weights_to_pickle(model, f'./results/variational_autoencoders/USL_pretrained/Attention/premodel_tcga_{omic}_vae_best_attention.pickle')

    print('\nVAE training completed in %.1f mins' % ((time.time() - start_time) / 60))

    model_save_name = f'premodel_tcga_{omic}_vae.pickle'
    save_weights_to_pickle(model, './results/variational_autoencoders/USL_pretrained/Attention/' + model_save_name)
    print("\nResults saved in /results/variational_autoencoders/USL_pretrained/Attention/%s\n\n" % model_save_name)
