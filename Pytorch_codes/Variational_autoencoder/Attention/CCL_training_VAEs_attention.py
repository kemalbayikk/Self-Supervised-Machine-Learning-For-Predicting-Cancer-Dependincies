...
if __name__ == '__main__':
    ...
    for data_type, data_ccl in data_dict.items():
        ...
        run = wandb.init(project="Self-Supervised-Machine-Learning-For-Predicting-Cancer-Dependencies", entity="kemal-bayik", name=f"SL_{data_type}_{ccl_size}CCL_{current_time}_attention")
        ...
        if data_type == 'mut':
            vae = load_pretrained_vae('results/variational_autoencoders/USL_pretrained/Attention/premodel_tcga_mut_vae_best_attention.pickle', tensor_data_ccl.shape[1], 1000, 100, 50)
        elif data_type == 'exp':
            vae = load_pretrained_vae('results/variational_autoencoders/USL_pretrained/Attention/premodel_tcga_exp_vae_best_attention.pickle', tensor_data_ccl.shape[1], 500, 200, 50)
        elif data_type == 'cna':
            vae = load_pretrained_vae('results/variational_autoencoders/USL_pretrained/Attention/premodel_tcga_cna_vae_best_attention.pickle', tensor_data_ccl.shape[1], 500, 200, 50)
        elif data_type == 'meth':
            vae = load_pretrained_vae('results/variational_autoencoders/USL_pretrained/Attention/premodel_tcga_meth_vae_best_attention.pickle', tensor_data_ccl.shape[1], 500, 200, 50)
        elif data_type == 'fprint':
            vae = VariationalAutoencoder(input_dim=tensor_data_ccl.shape[1], first_layer_dim=1000, second_layer_dim=100, latent_dim=50)
        ...
        # Save model weights
        save_weights_to_pickle(trained_vae, f'./results/variational_autoencoders/Attention/premodel_ccl_{data_type}_vae.pickle')
        run.finish()
