...
class LearnableGating(nn.Module):
    ...
    def forward(self, inputs):
        gated_inputs = [inp * self.gates[i] for i, inp in enumerate(inputs)]
        return gated_inputs, self.gates
...
class DeepDEP(nn.Module):
    ...
    def forward(self, mut, exp, cna, meth, fprint):
        ...
        inputs = [mu_mut, mu_exp, mu_cna, mu_meth, mu_fprint]
        gated_inputs, gates = self.gating(inputs)
        
        merged = torch.cat(gated_inputs, dim=1)
...
def train_model(model, train_loader, test_loader, num_epoch, patience, learning_rate, gate_learning_rate):
    criterion = nn.MSELoss()
    optimizer_gates = optim.Adam([model.gating.gates], lr=gate_learning_rate)
    optimizer_rest = optim.Adam([param for name, param in model.named_parameters() if 'gating.gates' not in name], lr=learning_rate)
    model.to(device)
    ...
    for epoch in range(num_epoch):
        ...
        for batch in progress_bar:
            ...
            training_gates_list.extend(gates.detach().cpu().numpy())
        ...
        # Gate ağırlıklarını yazdırma
        gate_values = gates.detach().cpu().numpy()
        vae_names = ["mut", "exp", "cna", "meth", "fprint"]
        gate_info = {vae: gate_values[i] for i, vae in enumerate(vae_names)}
        print(f"Gate Values at Epoch {epoch+1}: {gate_info}")
        ...
        wandb.log({
            ...
            "epoch": epoch + 1,
            "gate_values": gate_info  # gate değerlerini loglama
        })
        ...
        if test_loss < best_loss:
            ...
            torch.save(best_model_state_dict, 'best_model_vae_64batch_LSG_seperated_LR.pth')
            print("Model saved")
        else:
            epochs_no_improve += 1
...
if __name__ == '__main__':
    ...
    wandb.init(project="Self-Supervised-Machine-Learning-For-Predicting-Cancer-Dependencies", entity="kemal-bayik", name=f"Just_NN_{ccl_size}CCL_{current_time}_64batch_LSG_seperated_LR")

    config = wandb.config
    config.learning_rate = 1e-4
    config.gate_learning_rate = 1e-2
    ...
    best_model_state_dict, training_predictions, training_targets_list = train_model(
        model, train_loader, test_loader, config.epochs, config.patience, config.learning_rate, config.gate_learning_rate
    )
    ...
    # Save the best model
    torch.save(best_model_state_dict, 'results/models/deepdep_vae_model_64batch_LSG_seperated_LR.pth')
    ...
    np.savetxt(f'results/predictions/y_true_train_CCL_VAE_64batch_seperated_LR.txt', y_true_train, fmt='%.6f')
    np.savetxt(f'results/predictions/y_pred_train_CCL_VAE_64batch_seperated_LR.txt', y_pred_train, fmt='%.6f')
    np.savetxt(f'results/predictions/y_true_test_CCL_VAE_64batch_seperated_LR.txt', y_true_test, fmt='%.6f')
    np.savetxt(f'results/predictions/y_pred_test_CCL_VAE_64batch_seperated_LR.txt', y_pred_test, fmt='%.6f')
